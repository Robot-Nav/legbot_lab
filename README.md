# Legbot Lab/MuJoCo Sim2Real 四足机器人强化学习平台

> 基于 NVIDIA Isaac Lab / Isaac Sim 与 MuJoCo 的 Legbot 四足机器人 **Sim2Real** 研究与部署平台。
...
- [5. 快速开始](#5-快速开始)
  - [5.1 环境准备](#51-环境准备)
  - [5.2 训练 Legbot 速度跟踪策略](#52-训练-Legbot-速度跟踪策略)
  - [5.3 推理与导出](#53-推理与导出)
...
Legbot 是一款面向研究与教育的四足机器人平台，具备 12 个主动关节（3×4 腿），全身采用 RobStride（灵足）系列电机驱动。
本项目旨在为 Legbot 等四足机器人 提供一套开源、可复现的 **端到端强化学习 Sim2Real 工具链**：
...
> 详细的训练参数、MDP 设计、奖励函数与部署说明请见 [`Legbot_rl_lab/README.md`](./Legbot_rl_lab/README.md)。
...
```
Legbot_mujoco (moe+cts)/
├── Legbot_rl_lab/                    # Isaac Lab 训练 + C++ Sim2Real 部署
│   ├── source/unitree_rl_lab/      # Python 训练源码
│   │   ├── assets/robots/          # Legbot / Go2 机器人配置
│   │   ├── tasks/locomotion/       # 速度跟踪任务（MDP、奖励、课程学习）
│   │   ├── rl/                     # MoE+CTS 算法实现
│   │   └── utils/                  # 部署配置导出
│   ├── scripts/rsl_rl/             # train.py / play.py / train_moe_cts.py / play_moe_cts.py
│   ├── deploy/robots/Legbot/         # C++ 控制器（FSM + ONNX Runtime + RobStride 驱动）
...
│   └── config.yaml
│
├── Legbot/                           # 基于 motrix_envs 的 Legbot 导航环境（NumPy/MuJoCo）
│   ├── cfg.py
│   ├── Legbot_section001_np.py
│   └── xmls/                       # MuJoCo 场景与 Legbot 模型
│
...
```bash
cd Legbot_rl_lab
python scripts/rsl_rl/train_moe_cts.py --task Unitree-Legbot-Velocity-MoECTS --headless
```
...
训练日志保存在 `Legbot_rl_lab/logs/rsl_rl/unitree_Legbot_velocity/<timestamp>/`。
...
```bash
cd Legbot_rl_lab
python scripts/rsl_rl/play_moe_cts.py --task Unitree-Legbot-Velocity-MoECTS
```
...
1. 确认电机串口（默认 `/dev/ttyUSB0`、`/dev/ttyUSB1`）与 IMU 串口（默认 `/dev/ttyUSB2`）存在，并调整 [`deploy/robots/Legbot/config/config.yaml`](./Legbot_rl_lab/deploy/robots/Legbot/config/config.yaml) 中的 `motor_ids`、`zero_offsets` 与实际硬件一致。
2. 编译控制器（同上）。
3. 启动实机模式：

```bash
cd Legbot_rl_lab/deploy/robots/Legbot/build
./Legbot_ctrl --mode sim2real
```
...
| 子目录 | 内容 |
|--------|------|
| [`Legbot_rl_lab/`](./Legbot_rl_lab) | 强化学习训练、MoE+CTS 算法、C++ 部署控制器、Sim2Real 配置与测试。 |
| [`simulate/`](./simulate) | MuJoCo 物理仿真器，提供与实机一致的 DDS 通信接口。 |
| [`Legbot/`](./Legbot) | 基于 `motrix_envs` 的导航任务训练环境。 |
