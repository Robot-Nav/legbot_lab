...
from .cfg import LegBotSection001EnvCfg
...
@registry.env("legbot_navigation_section001", "np")
class LegBotSection001Env(NpEnv):
    """
    LegBot在Section001地形上的导航任务
    继承自NpEnv，使用LegBotSection001EnvCfg配置
    """
    _cfg: LegBotSection001EnvCfg
    
    def __init__(self, cfg: LegBotSection001EnvCfg, num_envs: int = 1):
        # 调用父类NpEnv初始化
    ...
    def reset(self, data: mtx.SceneData, done: np.ndarray = None) -> tuple[np.ndarray, dict]:
        cfg: LegBotSection01EnvCfg = self._cfg
        num_envs = data.shape[0]
