...
def _resolve_cts_feature_dims(num_single_obs: int, num_actions: int) -> list[int]:
    """Resolve the term-major CTS observation layout.

    The observation is ``ang_vel(3), gravity(3), command(3), joint_pos,
    joint_vel(num_actions), last_action(num_actions)``.  Wheel angles are
    intentionally omitted for wheel-legged robots because they are unbounded.
    """
    joint_pos_dim = num_single_obs - 9 - 2 * num_actions
    if joint_pos_dim <= 0 or joint_pos_dim > num_actions:
        raise ValueError(
            "Unsupported single_obs layout: expected 3+3+3+joint_pos_dim+"
            "num_actions+num_actions, with 0 < joint_pos_dim <= num_actions; "
            f"got num_single_obs={num_single_obs}, num_actions={num_actions}."
        )
    return [3, 3, 3, joint_pos_dim, num_actions, num_actions]
