...
# Ensure local imports take precedence over pip packages (e.g., cv2.utils)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    def _get_obs_dict(obs_result):
        """Get the full observation dict from the environment.

        Handles both the older IsaacLab API (get_observations() returns TensorDict)
        and the newer API (get_observations() returns tuple (obs, extras)).
        Also converts plain dict to TensorDict for compatibility with rsl_rl algorithms.
        """
        if isinstance(obs_result, tuple):
            # Newer IsaacLab API: (policy_obs, {"observations": full_obs_dict})
            obs = obs_result[1]["observations"]
        else:
            obs = obs_result
        # Convert plain dict to TensorDict if needed
        if not isinstance(obs, TensorDict):
            # Determine batch size from first observation tensor
