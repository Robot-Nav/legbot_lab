...
const RANGES = {
  ...
  kp: [0.0, 500.0],
  kd: [0.0, 5.0],
  tau: [-16.0, 16.0],            // hip/thigh: ±16 N·m
  calf_tau: [-32.0, 32.0],       // calf: ±32 N·m
};

function isCalfMotorId(motorId) {
  return (motorId % 10) === 3;
}

function tauRangeForMotor(motorId) {
  return isCalfMotorId(motorId) ? RANGES.calf_tau : RANGES.tau;
}
...
export function encodeStandardType1Frame({ channel, motorId, q, dq, kp, kd, tau }) {
  return encodeFrame({
    ...
    idField: floatToUint(tau, ...tauRangeForMotor(motorId)),
    canOrMaster: motorId,
...
export function decodeType1Command(frame) {
  ...
  const motorId = motorIdFromFrame(frame);
  return {
    motorId,
    ...
    tau: frame.frameType === STANDARD_FRAME ? uintToFloat(frame.idField, ...tauRangeForMotor(motorId)) : 0,
  };
}
...
export function decodeType2Feedback(frame) {
  ...
  return {
    ...
    q: uintToFloat(u16be(frame.data, 0), ...RANGES.q),
    dq: uintToFloat(u16be(frame.data, 2), ...RANGES.dq),
    tau: uintToFloat(u16be(frame.data, 4), ...tauRangeForMotor(motorId)),
