...
class StudentTeacher(nn.Module):
    ...
    学生网络使用普通观测（策略组）输出动作；教师网络使用特权观测（教师组）提供蒸馏目标。
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        if kwargs:
            print(
                'StudentTeacher.__init__ 收到未预期参数，将被忽略：'
        ...
        for obs_group in obs_groups['policy']:
            assert len(obs[obs_group].shape) == 2, 'StudentTeacher 模块仅支持 1D 观测。'
