...
复用 Go2 的 MDP 模块（奖励、观测、事件、课程学习、地形），
因为 LegBot 具有相同的 12 自由度运动学结构与关节命名约定。
"""
...
BASE_HEIGHT_TARGET = 0.28  # 目标躯干高度，与真实机器人质心高度对齐
...
class LegbotEnvCfg(ManagerBasedRLEnvCfg):
    ...
    scene: LegbotSceneCfg = LegbotSceneCfg(num_envs=4096, env_spacing=0.5)
