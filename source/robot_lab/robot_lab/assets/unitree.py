...
class UnitreeUrdfFileCfg(sim_utils.UrdfFileCfg):
    """Unitree URDF 资源文件配置，启用碰撞并关闭默认关节驱动。"""
    ...
    def replace_asset(self, meshes_dir, urdf_path):
        ...
        当需要修改碰撞体时，将修改后的 URDF 单独放在本仓库中，
        meshes_dir 由 unitree_ros 提供。
        本函数会在 ``/tmp`` 目录下自动构建完整的 robot_description 文件结构。
        注意：URDF 中的网格引用应与 URDF 处于同一目录层级。
        """
