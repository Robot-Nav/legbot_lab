...
class SimNorm(nn.Module):
    ...
    Args:
        dim: The dimension of each simplex group. The input's last dimension
             must be divisible by this value. Defaults to 8.
    """
    ...
    def forward(self, x):
        ...
        if shp[-1] % self.dim != 0:
            raise ValueError(
                f"SimNorm: input last dimension {shp[-1]} is not divisible by "
                f"simplex dimension {self.dim}. Ensure latent_dim is a multiple of {self.dim}."
            )
