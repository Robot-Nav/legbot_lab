...
from unitree_rl_lab.assets.robots.unitree import UNITREE_VBOT_CFG as ROBOT_CFG
...
class EventCfg:
    ...
    # -- reset action smoothness buffer on env reset --
    reset_action_smoothness = EventTerm(
        func=mdp.reset_action_smoothness_buffer,
        mode="reset",
    )
...
class CommandsCfg:
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        limit_ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-2.0, 2.0), lin_vel_y=(-1.0, 1.0), ang_vel_z=(-2.0, 2.0)
        ),
    )
...
class ObservationsCfg:
    ...
    class CriticCfg(ObsGroup):
        ...
        last_action = ObsTerm(func=mdp.last_action, clip=(-100, 100))
        # 足部接触力 norm (4,) —— 与原版 go2 privileged obs 一致，scale=1e-3
        foot_contact_forces = ObsTerm(
            func=mdp.foot_contact_force_norm, clip=(-100, 100), scale=1e-3,
            params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_foot")},
        )
        # 关节加速度 (12,) —— 与原版 go2 privileged obs 一致，scale=1e-4
        joint_acc = ObsTerm(func=mdp.joint_acc, scale=1e-4, clip=(-100, 100))
        height_measurements = ObsTerm(
            func=mdp.height_scan, clip=(-100, 100),
...
@configclass
class RewardsCfg:
    # -- tracking rewards (match go2 original weights) --
    ...
    # -- velocity penalties (match go2) --
    base_linear_velocity = RewTerm(func=mdp.lin_vel_z_l2, weight=-2.0)
    base_angular_velocity = RewTerm(func=mdp.ang_vel_xy_l2, weight=-0.05)

    # -- joint penalties (calibrated for vbot's 4x stiffness, 12x damping) --
    # vbot stiffness=80 vs go2=20 (4x), so torques are ~4x larger
    # Reduce torque penalty by 4x: -1e-4 -> -2.5e-5
    joint_vel = RewTerm(func=mdp.joint_vel_l2, weight=-0.001)
    joint_acc = RewTerm(func=mdp.joint_acc_l2, weight=-2.5e-7)
    joint_torques = RewTerm(func=mdp.joint_torques_l2, weight=-2.5e-5)
    action_rate = RewTerm(func=mdp.action_rate_l2, weight=-0.01)
    dof_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-2.0)
    energy = RewTerm(func=mdp.energy, weight=-2e-5)

    # -- orientation (go2 original doesn't use flat_orientation, keep small) --
    flat_orientation_l2 = RewTerm(func=mdp.flat_orientation_l2, weight=-0.0)

    # -- joint position penalty (reduced from -0.7 to -0.05, go2 doesn't have this) --
    joint_pos = RewTerm(
    ...
    # -- feet rewards (match go2: feet_air_time=1.0 is core gait reward) --
    ...
    # -- hip to default (match go2) --
    hip_to_default = RewTerm(
        func=mdp.hip_to_default,
        weight=-0.05,
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=".*"),
        },
    ...
    # -- CTS foot regulation (match go2, base_height_target calibrated for vbot 0.462 init) --
...
class TerminationsCfg:
    ...
    terrain_out_of_bounds = DoneTerm(
        func=mdp.terrain_out_of_bounds, params={"distance_buffer": 3.0}
    )
