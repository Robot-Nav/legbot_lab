...
inline po::variables_map helper(int argc, char** argv) 
{
    ...
    po::options_description desc("VBot Controller");
    desc.add_options()
        ("help,h", "produce help message")
        ("version,v", "show version")
        ("log", "record log file")
        ("network,n", po::value<std::string>()->default_value(""), "dds network interface")
        ("mode,m", po::value<std::string>()->default_value("sim2sim"), "mode: sim2sim or sim2real")
        ("can,c", po::value<std::string>()->default_value("can0"), "CAN interface for sim2real")
        ("serial,s", po::value<std::string>()->default_value("/dev/ttyUSB0"), "serial port for sim2real")
        ;
