...
class UnitreeActuator(DelayedPDActuator):
    ...
    def compute(
        ...
        # 最终安全网：确保输出力矩不含 NaN/inf
        self.applied_effort = torch.nan_to_num(self.applied_effort, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def _clip_effort(self, effort: torch.Tensor) -> torch.Tensor:
        # 清理输入中的 NaN/inf，防止通过 torch.where 传播
        effort = torch.nan_to_num(effort, nan=0.0, posinf=0.0, neginf=0.0)
        joint_vel = torch.nan_to_num(self._joint_vel, nan=0.0, posinf=0.0, neginf=0.0)
        # 判断力矩方向是否与关节速度一致
        ...
        max_effort = torch.where(
            joint_vel.abs() < self._velocity_x1, max_effort, self._compute_effort_limit(max_effort)
        )
        return torch.clip(effort, -max_effort, max_effort)

    def _compute_effort_limit(self, max_effort):
        # 清理输入中的 NaN/inf
        max_effort = torch.nan_to_num(max_effort, nan=0.0, posinf=0.0, neginf=0.0)
        joint_vel_abs = torch.nan_to_num(self._joint_vel.abs(), nan=0.0, posinf=0.0, neginf=0.0)
        k = -max_effort / (self._velocity_x2 - self._velocity_x1)
        limit = k * (joint_vel_abs - self._velocity_x1) + max_effort
...
@configclass
class UnitreeActuatorCfg_LegbotHipThigh(UnitreeActuatorCfg):
    X1 = 15.0
    X2 = 30.0
    Y1 = 15.0
    Y2 = 15.0
...
@configclass
class UnitreeActuatorCfg_LegbotCalf(UnitreeActuatorCfg):
    X1 = 10.0
    X2 = 20.0
    Y1 = 30.0
    Y2 = 30.0
