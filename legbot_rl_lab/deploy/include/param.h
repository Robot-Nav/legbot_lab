...
inline std::filesystem::path repo_root()
{
    ...
    std::filesystem::path p = proj_dir;
    for (int depth = 0; depth < 8 && !p.empty(); ++depth) {
        if (std::filesystem::exists(p / "legbot_rl_lab") &&
            std::filesystem::exists(p / "serial_dds_gateway")) {
            return p;
        }
...
inline po::variables_map helper(int argc, char** argv) 
{
    ...
    po::options_description desc("Legbot Controller");
    desc.add_options()
        ("help,h", "produce help message")
        ("version,v", "show version")
        ("log", "record log file")
        ("csv-log", "record 50Hz CSV to <legbotDog>/log/ (FixStand + Velocity)")
    ...
    if (vm.count("csv-log"))
    {
        ...
        std::filesystem::create_directories(csv_log_dir());
        spdlog::info("CSV logging enabled: {}/fixstand_*.csv and run_*.csv", csv_log_dir().string());
    }
