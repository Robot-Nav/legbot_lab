...
# ────────────────── MuJoCo 键盘快捷键抑制（仿 C++ main.cc user_key_cb） ──────────────────
#
# 关键问题：Linux 上 launch_passive 将渲染循环跑在守护线程，GLFW 上下文不在主线程。
# 主线程调用 glfw.get_current_context() 返回 None，无法设置 key callback。
#
# 解决方案：通过 launch_passive(key_callback=...) 在渲染线程内获取窗口并设置回调。
# 该回调首次触发时保存 GLFW 窗口到全局变量，并安装真正的 GLFW key callback
# 来屏蔽 WASD 等冲突按键。KeyboardController 随后用该窗口做 glfwGetKey() 轮询。

import threading as _threading
import mujoco.glfw as _mujoco_glfw

_GLFW = _mujoco_glfw.glfw  # 底层 glfw Python 绑定

# 需要屏蔽的按键
_SUPPRESS_KEYS = frozenset({
    _GLFW.KEY_W, _GLFW.KEY_A, _GLFW.KEY_S, _GLFW.KEY_D,
    ...
    _GLFW.KEY_TAB, _GLFW.KEY_ENTER,
    _GLFW.KEY_1, _GLFW.KEY_2,
    _GLFW.KEY_F, _GLFW.KEY_G, _GLFW.KEY_C, _GLFW.KEY_V, _GLFW.KEY_Z, _GLFW.KEY_X,
})

# 从渲染线程获取的 GLFW 窗口引用，及就绪信号
_glfw_window = None
_glfw_window_ready = _threading.Event()

# 保存 MuJoCo 内部原始 key callback
_mujoco_original_key_callback = None


def _mujoco_key_filter(win, key, scancode, action, mods):
    """GLFW key callback（运行于渲染线程）：屏蔽冲突按键，其余转发给 MuJoCo。"""
    if key in _SUPPRESS_KEYS:
        return  # 吞掉，不传给 MuJoCo
    if _mujoco_original_key_callback is not None:
        _mujoco_original_key_callback(win, key, scancode, action, mods)


def _launch_key_callback(key_code: int) -> None:
    """传给 launch_passive 的 key_callback（运行于渲染线程）。

    首次触发时：获取 GLFW 窗口 → 保存原始回调 → 安装键抑制回调 → 通知主线程。
    之后的触发：不做任何事（真正的键处理已由 _mujoco_key_filter 接管）。
    """
    global _glfw_window, _mujoco_original_key_callback
    ...
    _glfw_window = win
    _mujoco_original_key_callback = _GLFW.set_key_callback(win, _mujoco_key_filter)
    _glfw_window_ready.set()
    print("✓ MuJoCo 快捷键抑制已启用（WASD, QE, 方向键等已屏蔽）")


def get_launch_key_callback():
    """返回可传给 launch_passive(key_callback=...) 的回调函数。"""
    return _launch_key_callback


def suppress_mujoco_keys():
    """（兼容旧接口）等待 GLFW 窗口就绪并确保键抑制已安装。

    新的键抑制机制通过 launch_passive(key_callback=...) 自动触发，此函数
    仅作为保险：如果窗口尚未就绪则等待，已就绪则无操作。
    """
    _glfw_window_ready.wait(timeout=2.0)


class KeyboardController:
    """键盘速度控制器。使用 glfwGetKey() 轮询（仿 C++ KeyboardJoystick）。

    通过渲染线程共享的 _glfw_window 读取按键状态，不依赖主线程 GLFW 上下文。
    I/K = 前进/后退    J/L = 左/右平移    U/O = 左/右旋转    M = 急停
    选用不与 MuJoCo 快捷键冲突的按键。
    """

    def __init__(self, max_cmd: np.ndarray, cmd_init: np.ndarray = None, step: float = 0.1):
        self._glfw = _GLFW
        ...
        self._key_map = {
            self._glfw.KEY_UP: np.array([1, 0, 0], dtype=np.float32),
            self._glfw.KEY_DOWN: np.array([-1, 0, 0], dtype=np.float32),
            self._glfw.KEY_LEFT: np.array([0, -1, 0], dtype=np.float32),
            self._glfw.KEY_RIGHT: np.array([0, 1, 0], dtype=np.float32),
            self._glfw.KEY_U: np.array([0, 0, 1], dtype=np.float32),
            self._glfw.KEY_O: np.array([0, 0, -1], dtype=np.float32),
        }
    ...
    def update(self) -> np.ndarray:
        ...
        if self._glfw.get_key(win, self._glfw.KEY_M) == self._glfw.PRESS:
            self.cmd[:] = 0.0
            return self.cmd.copy()

        delta = np.zeros(3, dtype=np.float32)
        for key_code, direction in self._key_map.items():
            if self._glfw.get_key(win, key_code) == self._glfw.PRESS:
                delta += direction

        if delta.any():
            self.cmd += delta * self.step
            self.cmd = np.clip(self.cmd, -self.max_cmd, self.max_cmd)
        return self.cmd.copy()
