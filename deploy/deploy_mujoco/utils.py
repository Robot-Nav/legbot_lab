...
try:
    import imageio
except ImportError:  # Optional: only required when recording video.
    imageio = None

try:
    import pygame
except ImportError:  # Optional: keyboard/default commands still work without a joystick.
    pygame = None
...
def load_config(config_name: str):
    ...
    data = {
        ...
        "num_leg_joints": int(raw.get("num_leg_joints", raw["num_actions"])),
    ...
    if not 0 < data["num_leg_joints"] <= data["num_actions"]:
        raise ValueError(
            f"Invalid joint dimensions: num_leg_joints={data['num_leg_joints']}, "
            f"num_actions={data['num_actions']}."
        )
    ...
    if "mujoco_joint_names" in raw and "model_joint_names" in raw:
        ...
        if len(mj_names) != data["num_actions"] or len(model_names) != data["num_actions"]:
            raise ValueError("Joint-name lists must both have num_actions entries.")
        if len(set(mj_names)) != len(mj_names) or set(mj_names) != set(model_names):
            raise ValueError("MuJoCo and model joint-name lists must contain the same unique names.")
...
def init_joystick():
    if pygame is None:
        print("pygame is not installed. Using default commands from config.")
        return None
    pygame.init()
...
def open_video_writer(enabled: bool, *, policy_path: Path, cmd: np.ndarray,
    ...
    if not enabled:
        return None, 1, None
    if imageio is None:
        raise ImportError("Video recording requires imageio. Install imageio or set save_video: false.")
