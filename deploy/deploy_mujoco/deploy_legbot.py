"""Run CTS LegBot policy deployment in MuJoCo with joystick command input.
...
Notes:
    This script currently uses CONFIG_NAME = "legbot.yaml" and does not expose CLI flags.
"""
...
def main() -> None:
    ...
    keyboard = KeyboardController(cfg.max_cmd)
    print("Keyboard control: Up/Down=forward/back  Left/Right=turn  U/O=yaw  M=stop")
    print("Robot starts stationary. Press keys to move.")
    ...
    with mujoco.viewer.launch_passive(
        model, data,
        key_callback=get_launch_key_callback(),
        show_left_ui=False, show_right_ui=False,
    ) as viewer:
        setup_tracking_camera(viewer)
        # 等待渲染线程初始化 GLFW 窗口（供 KeyboardController 轮询使用）
        suppress_mujoco_keys()
